  ...
  const infoPurchase = (): string => {
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow-md">
        <h3 className="text-zinc-600 font-semibold">{formatDate(purchase.createdAt)}</h3>
        <hr />
        <h2 className="text-xl text-zinc-400 font-bold">{purchase.status}</h2>
        <h2>{infoPurchase()}</h2>
        <div className="flex items-center mt-2">
          {purchase.purchaseProducts.map((product) => (
            <div key={product.productId} className="bg-white p-4 rounded-lg shadow-md">
              ...
              <div className="flex items-center mt-2">
                <span className="text-zinc-400 mx-2">{product.quantity}</span>

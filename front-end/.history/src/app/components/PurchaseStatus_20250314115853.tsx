...
export default function PurchaseStatus({ purchase }: PurchaseStatusProps) {
  ...
  const getStatusColor = (status: string): string => {
    switch (status) {
      ...
      default:
        return 'text-zinc-400'; // Cor padrão (cinza)
    }
  };
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow-md">
        ...
        <hr />
        <h2 className={`text-xl font-bold mt-2 ${getStatusColor(purchase.status)}`}>{purchase.status}</h2>
      ...
      >
        {showProducts ? 'Produtos 🔼' : 'Produtos 🔽'}
      </button>
